...
const ListVariationTemplatePageComponent: React.FC = () => {
	...
	const [link, setLink] = useState(`${BACKENDAPI}/v1.0/variation-templates`);
	...
	const deleteData = (id: number) => {
		if (window.confirm("Are you sure  want to delete ?")) {
			apiClient()
				.delete(`${BACKENDAPI}/v1.0/variation-templates/${id}`)
				.then((response: any) => {
						...
						{data.map((el: any) => {
							return (
								<tr key={el.id}>
									...
									<td>{el.variation_value_template && (
										<>{
										el.variation_value_template.map((el:any) => {
											return (<>
											{el.name} {" "}
											</>)
										})
...
export default ListVariationTemplatePageComponent;
