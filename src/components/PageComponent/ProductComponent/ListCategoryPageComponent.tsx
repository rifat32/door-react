...
const ListCategoryPageComponent: React.FC = () => {
	...
	const [link, setLink] = useState(`${BACKENDAPI}/v1.0/categories`);
	...
	const deleteData = (id: number) => {
		if (window.confirm("Are you sure  want to delete ?")) {
			apiClient()
				.delete(`${BACKENDAPI}/v1.0/categories/${id}`)
				.then((response: any) => {
	...
	const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
		if(e.target.value) {
			setLoading(true)
			apiClient()
				.get(`${BACKENDAPI}/v1.0/categories/search/${e.target.value}`)
				.then((response: any) => {
					setLoading(false)
					console.log(response.data.data);
...
export default ListCategoryPageComponent;
