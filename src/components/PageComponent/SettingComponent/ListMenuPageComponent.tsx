...
import AddMenuForm from "../../Forms/SettingForms/AddMenuForm";



const ListMenuPageComponent: React.FC = () => {
	...
	const [link, setLink] = useState(`${BACKENDAPI}/v1.0/menus`);
	...
	const deleteData = (id: number) => {
		if (window.confirm("Are you sure  want to delete ?")) {
			apiClient()
				.delete(`${BACKENDAPI}/v1.0/menus/${id}`)
				.then((response: any) => {
	...
	const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
		if(e.target.value) {
			setLoading(true)
			apiClient()
				.get(`${BACKENDAPI}/v1.0/menus/search/${e.target.value}`)
				.then((response: any) => {
					setLoading(false)
					console.log(response.data.data);
...
export default ListMenuPageComponent;
