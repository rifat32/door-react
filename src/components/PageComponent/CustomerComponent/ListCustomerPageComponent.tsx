...
import AddCategoryForm from "../../Forms/ProductForms/AddCategoryForm";
import { withRouter } from "react-router-dom";



const ListCutomerPageComponent: React.FC = (props:any) => {
	...
	const [link, setLink] = useState(`${BACKENDAPI}/v1.0/customers`);
	...
	const deleteData = (id: number) => {
		if (window.confirm("Are you sure  want to delete ?")) {
			apiClient()
				.delete(`${BACKENDAPI}/v1.0/customers/${id}`)
				.then((response: any) => {
	...
	const viewCustomer = (id: any) => {
		props.history.push(`/admin/customers/${id}`);
	  };
	  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
		if(e.target.value) {
			setLoading(true)
			apiClient()
				.get(`${BACKENDAPI}/v1.0/customers/search/${e.target.value}`)
				.then((response: any) => {
					setLoading(false)
					console.log(response.data.data);
...
export default withRouter(ListCutomerPageComponent);
