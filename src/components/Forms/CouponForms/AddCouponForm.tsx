...
const AddCouponForm: React.FC<UpdateFormInterface> = (props) => {
  ...
  const loadData = (urlOrPerPage: number | string, select = true) => {
    ...
    if (select) {
      ...
      setSelectedData((prevData: any) => {
        return [...prevData, ...tempSelectedData];
      });
    } else {
      setSelectedData([]);
    }
  ...
  const setLinksView = (el: Links, index: number, arr: object[]) => {
	  if(el.url){
		let params =
		`${BACKENDAPI}/v1.0/products/pagination/${perPage}?page=${current_page}&&category=${formData.category_id}`.split(
		  "?"
		)[1];
        ...
        return (
          <React.Fragment key={index}>
            <li className="page-item">
              <button
                type="button"
                className={`page-link  ${el.active && "text-dark"}`}
                onClick={() =>
  ...
  return (
    <form className="row g-3" onSubmit={handleSubmit}>
        ...
        <div className="col-md-4">
          ...
          {errors?.is_all_category_product && (
            <div className="invalid-feedback">
              {errors.is_all_category_product[0]}
            </div>
          )}
          {errors && <div className="valid-feedback">Looks good!</div>}
        </div>
      ...
      {formData.category_id && parseInt(formData.is_all_category_product) === 0 ? (
        <div className="row">
          ...
          {!data?.length ? (
            <div className="noProduct d-flex align-items-center justify-content-center">
              {loading ? (
                ...
                <h3 className="display-3">No products to show</h3>
              )}
            </div>
          ) : (
            <React.Fragment>
